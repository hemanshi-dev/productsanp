...
const AIImageGenerator = () => {
  ...
  // Reset everything (form data, generated images, and center panel)
  const resetEverything = () => {
    // Reset form data
    resetFormData()

    // Clear generated images and data
    setGeneratedImages([])
      ...
      if (response.data) {
        ...
        if (dataStatus === 'completed' || dataStatus === 'partial_success') {
          ...
          if (response.data.generated_images && response.data.generated_images.length > 0) {
            ...
            // Switch to center view on mobile when images are generated
          ...
          let errorMessage = 'Too much traffic at the moment. Try again in a while.'
            ...
            if (response.data.generated_videos && response.data.generated_videos.length > 0) {
              ...
              if (videoUrl) {
                ...
                if (videoPlaceholderIndex !== -1) {
                  ...
                  const videoIndex = generatedImagesData.findIndex((img) => img.id === imageId && img.type !== 'video')
                  if (videoIndex !== -1) {
  ...
  const handleGenerateVideo = async (imageId: string, e?: React.MouseEvent) => {
    ...
    try {
      ...
      toast.error('Too much traffic at the moment. Try again in a while.')
      setVideoGenerating(prev => {
        const newState = { ...prev }
        delete newState[imageId]
        return newState
  ...
  const handleGenerate = async () => {
    ...
    // Switch to center view on mobile when generate is clicked
    setMobileView('center')
      ...
      <div className="min-h-screen bg-gray-900 text-white">
        <div className="min-h-screen flex flex-col">
          ...
          >
               ...
               {/* Categories in Top Bar - Hidden on mobile */}
              <div className="hidden md:flex w-6/12 items-center gap-2 overflow-x-auto flex-1 justify-center">
                {loadingCategories ? (
                  <div className="animate-spin rounded-full h-4 w-4 border-2 border-[var(--color-primary)]/80 border-t-[var(--color-primary)]"></div>
                ) : (
                  <div className="flex items-center gap-2 border border-white/20 rounded-full p-2">
          ...
          <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            } ${isGenerating || generatedImages.length > 0 ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className="p-6">
                <h2 className="text-lg font-semibold text-white mb-1">Inputs</h2>
                
                {/* Subcategories (Banners) */}
                <div className="mb-6">
                  <label className="block text-sm font-medium text-white mb-3">What you’re creating
                  </label>
                  {loadingBanners ? (
                    ...
                    <p className="text-xs text-white/70">No subcategories available</p>
                  ) : (
                    <div className="space-y-2">
                      {banners.map((banner) => (
                        <button
                          ...
                          className={`w-auto display-inline mr-2 text-center py-2 px-2.5 rounded-lg text-xs transition-all ${
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-gradient-to-r from-gray-700/50 to-gray-800/50  border border-white/30 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-[var(--color-primary)] transition-all"
                ...
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-sm font-medium text-white">Additional Image</label>
                    {/* {selectedCategory?.id === '693667ca799d1d1e0cb46b9a' && (
                      <button
                        onClick={() => setShowImageInstructionModal(true)}
                        className="text-xs text-primary hover:underline"
              ...
              <div className="w-full h-full">
                ...
                {isGeneratingVideo && videoJobId && videoSourceImageUrl ? (
                    ...
                    <div className="relative bg-gray-900 rounded-xl border-2 border-white/20 overflow-hidden shadow-lg max-w-2xl mx-auto">
                      <img
                        ...
                        onError={(e) => {
                          ...
                          const hasRetried = img.dataset.retried === 'true'
                          if (!hasRetried) {
                            img.dataset.retried = 'true'
                            img.src = `${videoSourceImageUrl}`
                          }
                        }}
                      />
                      ...
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                        <div className="text-center">
                          <div className="relative w-16 h-16 mx-auto mb-4">
                            <div className="animate-spin rounded-full h-16 w-16 border-4 border-gray-700/50"></div>
                  ...
                  <>
                    <div className="flex flex-wrap justify-center gap-2">
                      {generatedImages.map((imgUrl, index) => {
                        ...
                        <div
                            key={index}
                            className="md:w-[calc(33.33%-0.5rem)] w-full relative group rounded-xl overflow-hidden border-2 border-white/20 hover:border-[var(--color-primary)] transition-all cursor-pointer"
                          ...
                          >
                            ...
                            {imageData?.gallery_image_id && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation()
                                  setSelectedImageIndex(null)
                                  handleGenerateVideo(imageData?.gallery_image_id || '', e)
                                }}
                                disabled={videoGenerating[imageData?.gallery_image_id]}
                                className="absolute top-12 w-[36px] h-[36px] right-2 z-[60] bg-black/70 hover:bg-black/90 backdrop-blur-sm rounded-lg p-2 transition-all cursor-pointer shadow-lg pointer-events-auto"
                              >
                                {videoGenerating[imageData?.gallery_image_id] ? (
                                  <div className="animate-spin rounded-full h-5 w-5 border-2 border-gray-700/50 border-t-white"></div>
                                ) : (
                                  <>
                  ...
                  <div className="w-full">
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                      {Array.from({ length: numberOfImages }).map((_, arrayIndex) => {
                        ...
                        return (
                          <div key={arrayIndex} className="md:w-[calc(33.33%-0.5rem)] w-full relative bg-gray-900 rounded-xl border-2 border-white/20 overflow-hidden aspect-square group">
                            {imageToShow ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center text-gray-900">
                                <div className="relative w-16 h-16">
                                  ...
                                  <div className="animate-spin rounded-full h-16 w-16 border-4 border-transparent border-t-white/80 absolute top-0 left-0" style={{ animationDirection: 'reverse' }}></div>
                                </div>
                              </div>
                            )}
                            {imageToShow && (
                          ...
                          ''
                              ...
                              <div className="absolute inset-0 bg-primary/5 flex items-center justify-center pointer-events-none">
                                <span className="text-xs font-medium text-white bg-gray-900/80 px-2 py-1 rounded">Processing...</span>
                              </div>
                            ) : (
                              <div className="absolute inset-0 bg-primary/5 flex items-center justify-center pointer-events-none">
                                <span className="text-xs font-medium text-white bg-gray-900/80 px-2 py-1 rounded">Waiting...</span>
                              </div>
                            )}
                          </div>
                        )
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-xl font-semibold text-white/80 mb-2">Product Snap AI Canvas</h3>
                    <p className="text-white/80 text-sm">Your generated images will appear here. Begin by adding a main image in the left panel.</p>
                  </div>
                )}
                 {/* Create New Button - Show after successful generation */}
                 {!isGenerating && !isGeneratingVideo && generatedImages.length > 0 && (
                    <button
                      onClick={resetEverything}
                      className="w-auto mx-auto mt-8 px-6 py-3 bg-gradient-to-r from-gray-700/50 to-gray-800/50 border border-white/20 text-white rounded-lg font-medium hover:bg-white/10 transition-all flex items-center justify-center gap-2"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                      </svg>
              ...
              <div className="md:p-6 px-6 space-y-6">
                ...
                <div>
                  <button
                    ...
                    className="w-full flex items-center justify-between mb-3 text-sm font-medium text-white"
                  >
                    <span>Output</span>
                  ...
                  {expandedSections.output && (
                    <div className="space-y-4">
                      ...
                      {/* Number of Images */}
                      <div>
                        <label className="block text-xs font-medium text-white mb-2">Number of Images</label>
                ...
                <div>
                  ...
                  {expandedSections.model && (
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs font-medium text-white mb-2">Images with Model</label>
                        <div className="flex gap-2">
                          <button
                      ...
                      {formData.imagesWithModel === 'yes' && (
                        <div className="space-y-3">
                          <button
                            ...
                            className="w-full px-4 py-2.5 bg-gradient-to-r from-gray-700/50 to-gray-800/50 text-gray-400  hover:bg-white/30 rounded-lg text-sm font-medium flex items-center justify-between"
                          >
                          ...
                          {formData.selectedFaceId && faces.find(f => f.id === formData.selectedFaceId) && (
                            <div className="relative w-full rounded-lg overflow-hidden border border-white/20 text-gray-400">
                              <img
                          ...
                          <label htmlFor="custom-face-upload" className="cursor-pointer block">
                            <div className="w-full px-4 py-2.5 border border-white/20 bg-gradient-to-r from-gray-700/50 to-gray-800/50 text-gray-400 hover:bg-white/30 rounded-lg text-sm font-medium text-center">
                              Upload Your Own
                            </div>
                          </label>
                          {customFacePreview && (
                  ...
                  {expandedSections.prompt && (
                    <div>
                      <textarea
                        ...
                        rows={4}
                        className="w-full px-4 py-2.5 bg-gradient-to-r from-gray-700/50 to-gray-800/50  border border-white/30 rounded-lg text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-[var(--color-primary)] transition-all resize-none"
                ...
                <div className="pt-4 border-t border-white/20">
                  <div className="flex items-center justify-between ">
                    <span className="text-xs font-medium text-white">Credits Use</span>
                    {loadingCredits ? (
                      <div className="animate-spin rounded-full h-4 w-4 border-2 border-[var(--color-primary)]/30 border-t-[var(--color-primary)]"></div>
                    ) : (
                  ...
                  <p className='text-xs text-white'>Per Image: 10 credits</p>
                  <p className='text-xs text-white'>Per Video: 20 credits</p>
                </div>
          ...
          <div className="space-y-6">
              ...
              <div className="border-2 rounded-2xl p-6" style={{borderColor: 'color-mix(in srgb, var(--color-primary) 30%, transparent)'}}>
                ...
                <input
                  ...
                  value={formData.productName}
                  onChange={(e) => setFormData({ ...formData, productName: e.target.value })}
                  className="w-full px-5 py-4 bg-secondary border border-black/50 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-[var(--color-primary)]/50 focus:ring-2 focus:ring-primary/20 transition-all"
                  placeholder="Enter product name"
                />
              </div>
                    ...
                    <div
                      ...
                      className={`border-2 border-dashed rounded-xl p-6 transition-all duration-300 ${
                        isDraggingMain
                          ...
                          : 'border-white/30 hover:border-[var(--color-primary)] bg-primary/10'
                      }`}
                    >
                      {mainImagePreview ? (
              ...
              <div className="border-2 rounded-2xl p-6" style={{borderColor: 'color-mix(in srgb, var(--color-primary) 30%, transparent)'}}>
                ...
                <select
                  ...
                  className="w-full px-5 py-4 bg-secondary border border-black/50 rounded-xl text-white focus:outline-none focus:border-[var(--color-primary)]/50 focus:ring-2 focus:ring-primary/20 transition-all"
                >
                  {[1, 2, 3, 4].map((num) => (
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    className={`flex-1 px-6 py-3 rounded-xl font-medium transition-all ${formData.imagesWithModel === 'yes'
                      ? 'bg-primary text-white'
                      : 'bg-secondary border border-white/30 text-white hover:border-[var(--color-primary)]/50'
                      }`}
                  >
                    Yes
                  </button>
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <button
                            type="button"
                            onClick={() => setShowFaceModal(true)}
                            className="w-full px-5 py-4 bg-secondary border border-white/30 rounded-xl hover:border-[var(--color-primary)]/50 transition-all text-left flex items-center justify-between"
                          >
                            <div className="flex items-center gap-3">
                              {formData.selectedFaceId ? (
                                <>
                                  ...
                                  />
                                  <span className="text-white text-sm">Model Selected</span>
                                </>
                              ) : (
                                <span className="text-white text-sm">Click to select a model</span>
                              )}
                            </div>
                              ...
                              >
                                {customFacePreview ? (
                                  ...
                                  <div className="text-center py-6">
                                    ...
                                    <p className="text-xs text-white mt-1">No file selected</p>
                                  </div>
                                )}
                              </div>
                            </label>
              ...
              <div className="border-2 rounded-2xl p-6" style={{borderColor: 'color-mix(in srgb, var(--color-primary) 30%, transparent)'}}>
                ...
                <textarea
                  ...
                  onChange={(e) => setFormData({ ...formData, customPrompt: e.target.value })}
                  rows={4}
                  className="w-full px-5 py-4 bg-secondary border border-black/50 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-[var(--color-primary)]/50 focus:ring-2 focus:ring-primary/20 transition-all resize-none"
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    className={`cursor-pointer px-4 py-2 rounded-lg font-medium transition-all text-sm ${
                      formData.aspectRatio === '1:1'
                        ? 'bg-primary border-2 border-[var(--color-primary)] text-white'
                        : 'border border-gray-700/50 text-white hover:border-gray-600/50'
                    }`}
                  >
                    1:1
                  </button>
        ...
        {showImageInstructionModal && (
          ...
          >
            ...
            >
              {/* Header */}
              <div className="flex items-center justify-between p-6 border-b border-white/20 bg-gradient-to-r from-primary/5 to-primary/10">
                <h2 className="text-2xl font-bold text-black">Image Upload Guidelines</h2>
                <button
                  onClick={() => setShowImageInstructionModal(false)}
                  className="w-10 h-10 rounded-full border border-white/30 hover:border-[var(--color-primary)] hover:bg-primary/10 transition-all text-black flex items-center justify-center"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    ...
                    <div className="relative rounded-lg overflow-hidden border-2 border-green-500/30">
                      <div className="aspect-square text-gray-900 flex items-center justify-center overflow-hidden">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    ...
                    {/* Example 2 */}
                    <div className="relative rounded-lg overflow-hidden border-2 border-red-500/30">
                      <div className="aspect-square text-gray-900 flex items-center justify-center">
        ...
        {showFaceModal && (() => {
          ...
          <div 
            ...
            onWheel={(e) => e.stopPropagation()}
          >
              <div className="relative w-full max-w-5xl h-[90vh] bg-gray-900 text-white rounded-2xl overflow-hidden flex flex-col border-2 border-gray-400/20">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-gray-300">
                  <div>
                    <h2 className="text-2xl font-bold mb-2">Select Model</h2>
                    <p className="text-sm text-gray-400">Choose a model for your image generation</p>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="w-10 h-10 rounded-full bg-gray-200 border border-white/30 hover:border-[var(--color-primary)] hover:bg-primary/10 transition-all text-black flex items-center justify-center"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                    ...
                    {['all', 'Men', 'Women', 'Kid'].map((filter) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium transition-all ${faceGenderFilter === filter
                          ...
                          : 'bg-gradient-to-r from-gray-700/50 to-gray-800/50 border border-white/20 text-gray-400 hover:border-[var(--color-primary)]/50'
                          }`}
                      >
                ...
                >
                  {loadingFaces ? (
                              ...
                              {facesByGender.Men.map((face) => (
                                <div
                                  ...
                                  className={`relative cursor-pointer rounded-lg overflow-hidden border-2 transition-all aspect-square ${selectedFaceInModal?.id === face.id
                                    ...
                                    : 'border-white/30 hover:border-[var(--color-primary)]/50'
                                    }`}
                                >
                                  <img
                                        src={face.image}
                    ...
                    filteredFaces.length === 0 ? (
                      <div className="text-center py-20">
                        <p className="text-gray-400">No models available for this filter</p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                ...
                <div className="p-6 border-t border-white/30 flex gap-3">
                  <button
                    ...
                    className="flex-1 px-6 py-3 text-gray-400 border border-white/30 rounded-xl hover:border-[var(--color-primary)] hover:bg-primary/10 transition-all text-black font-semibold"
                  >
                    Cancel
                  </button>
                  <button
          ...
          return (
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => setSelectedImageIndex(null)}
                  className="absolute top-4 right-4 z-50 w-10 h-10 rounded-full bg-gray-900/90 text-white hover:bg-gray-900 border border-white/30 hover:border-[var(--color-primary)] transition-all text-black flex items-center justify-center shadow-lg"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                ...
                <div className="relative w-full max-w-4xl h-full max-h-[90vh] flex flex-col">
                  {!imageUrl ? (
                    <div className="flex-1 flex items-center justify-center p-8 text-center">
                      <div>
                        <p className="text-white text-lg mb-4">Image not available</p>
                        <p className="text-white text-sm mb-6">The image may still be processing or has been removed.</p>
                        <button
                          onClick={() => setSelectedImageIndex(null)}
                    ...
                    <>
                      ...
                      <div className=" p-2 bg-gray-900 text-white rounded-full w-auto mt-4 mx-auto flex items-center justify-center">
                        {imageData?.type === 'video' ? (
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                            ...
                            className="md:px-4 md:py-2 px-2 py-1 text-white transition-all text-xs font-medium flex items-center justify-center gap-2"
                          >
                          ...
                          <>
                            <button
                              ...
                              className="md:px-4 md:py-2 px-2 py-1 border-gray-700/50 border-r text-white transition-all text-xs font-medium flex items-center justify-center gap-2"
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 17V3"/><path d="m6 11 6 6 6-6"/><path d="M19 21H5"/></svg>
                              Download
                            </button>
                            {(imageId || galleryImageId) && (
                              <button
                                ...
                                className="md:px-4 md:py-2 px-2 py-1 border-gray-700/50 border-r text-white transition-all text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                              >
        ...
        <div ref={sectionRef} className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-12 gap-4">
            <div>
              ...
              <p className="text-white mt-3">Download or save your generated images</p>
            </div>
            <button
              onClick={() => {
                setCurrentStep('form')
                    ...
                    {imageData?.type === 'video' ? (
                      <>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <div className="w-12 h-12 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center border border-gray-700/50">
          ...
          return (
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => setSelectedImageIndex(null)}
                  className="absolute top-4 right-4 z-50 w-10 h-10 rounded-full bg-gray-900/90 hover:bg-gray-900 border border-white/30 hover:border-[var(--color-primary)] transition-all text-black flex items-center justify-center shadow-lg"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                ...
                {/* White Background Container with Centered Image/Video */}
                <div className="relative w-full max-w-4xl h-full max-h-[90vh] bg-gray-900 rounded-lg shadow-2xl flex flex-col">
                  {!imageUrl ? (
                    <div className="flex-1 flex items-center justify-center p-8 text-center">
                      <div>
                        <p className="text-white text-lg mb-4">Image not available</p>
                    ...
                    <>
                      ...
                      {/* Action Buttons - Fixed at Bottom */}
                      <div className="border-t border-white/20 p-4 flex items-center justify-center gap-3">
                        {imageData?.type === 'video' ? (
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
          ...
          >
            ...
            <div className="p-6 border-t border-white/30 flex gap-3">
              <button
                ...
                className="flex-1 px-6 py-3 bg-gray-200 border border-white/30 rounded-xl hover:border-[var(--color-primary)] hover:bg-primary/10 transition-all text-black font-semibold"
              >
                Cancel
              </button>
              <button
