...
import { createPrompt, fetchPrompts, resetPagination } from '../../store/slices/promptsSlice'
...
const AddPrompt = () => {
  ...
  const onSubmit = async (data: PromptFormData) => {
    ...
    if (!customPrompt1.trim() || !customPrompt2.trim() || !customPrompt3.trim() || !customPrompt4.trim()) {
      toast.error('All 4 Custom Prompts for Different Angles are required')
      setSubmitting(false)
      return
    }
    ...
    try {
      ...
      const result = await dispatch(createPrompt(formData))

      if (createPrompt.fulfilled.match(result)) {
        toast.success('Prompt created successfully')
        // Refresh the prompts list before navigating
        dispatch(resetPagination())
        dispatch(fetchPrompts({ page: 1, cursor: undefined }))
        navigate('/admin/prompts')
      } else {
        toast.error(result.payload as string || 'Failed to create prompt')
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-bold text-white mb-2 tracking-tight">Add New Prompt</h1>
        <p className="text-gray-400 text-lg">Create a new prompt</p>
      </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                type="button"
                onClick={addPrompt}
                className="px-4 py-2 text-sm bg-primary  text-white rounded-full transition-all duration-300 hover:bg-[var(--color-primary-light)] hover:text-black font-semibold flex items-center gap-2"
              >
