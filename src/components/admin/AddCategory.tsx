...
import { createCategory, fetchCategories, resetPagination } from '../../store/slices/categoriesSlice'
  ...
  const onSubmit = async (data: CategoryFormData) => {
    ...
    try {
      ...
      if (createCategory.fulfilled.match(result)) {
        ...
        // Refresh the categories list before navigating
        dispatch(resetPagination())
        dispatch(fetchCategories({ page: 1, cursor: undefined }))
        navigate('/admin/categories')
      } else {
        toast.error(result.payload as string || 'Failed to create category')
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-bold text-white mb-2 tracking-tight">Add New Category</h1>
        <p className="text-gray-400 text-lg">Create a new category for your content</p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              {imagePreview && (
                <div className="relative w-[150px] h-[150px] rounded-sm overflow-hidden border border-gray-700/50">
                  <img
                    src={imagePreview}
                    alt="Preview"
          ...
          <div>
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <div className={`w-14 h-8 rounded-full transition-all duration-300 ${
                showBanner ? 'bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-light)]' : 'bg-gray-700'
              }`}>
